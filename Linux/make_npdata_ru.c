...
bool decrypt(int hash_mode, int crypto_mode, int version, unsigned char *in, unsigned char *out, int length, unsigned char *key, unsigned char *iv, unsigned char *hash, unsigned char *test_hash)
{
	...
	{
		printf("Ошибка: Неизвестный криптоалгоритм!\n");
		return false;
	}
	...
	{
		printf("Ошибка: Неизвестный алгоритм хэширования!\n");
		return false;
	}
}
...
int decrypt_data(FILE *in, FILE *out, EDAT_HEADER *edat, NPD_HEADER *npd, unsigned char* crypt_key, bool verbose)
{
	...
	for (i = 0; i < block_num; i++)
	{
		...
		else
		{
			...
			{
				if (verbose)
					printf("ВНИМАНИЕ: Блок по адресу 0x%llx имеет не верный хэш!\n", offset);
		...
		if (((edat->flags & EDAT_COMPRESSED_FLAG) != 0) && compression_end)
		{
			...
			if (verbose)
			{
				printf("Размер сжатого блока: %d\n", pad_length);
				printf("Размер распакованного блока: %d\n", res);
			}
			...
			if (edat->file_size == 0)
			{
				if (res < 0)
				{
					printf("Не удалось распаковать EDAT/SDAT!\n");
					return 1;
				}
				else
					printf("EDAT/SDAT успешно распакован!\n");
			}
...
int check_data(unsigned char *key, EDAT_HEADER *edat, NPD_HEADER *npd, FILE *f, bool verbose)
{
	...
	if ((npd->version == 0) || (npd->version == 1))
	{
		...
		{
			printf("Ошибка: Bad header flags!\n");
			return 1;
		}
	}
	...
	else if ((npd->version == 3) || (npd->version == 4))
	{
		...
		{
			printf("Ошибка: Не верные флаги в заголовке!\n");
			return 1;
		}
	}
	else
	{
		printf("Ошибка: Неизвестная версия!\n");
		return 1;
	}
	...
	if ((edat->flags & EDAT_DEBUG_DATA_FLAG) != 0)
	{
		...
		if (verbose)
			printf("Обнаружены данные Дебага!\n");
	}
	...
	{
		if (verbose)
			printf("ВНИМАНИЕ: Не верный хэш заголовка!\n");
		...
		{
			printf("Ошибка: Не верный ключ RAP/RIF/KLIC!\n");
			return 1;
		}
	}
	...
	{
		if (verbose)
			printf("Обнаружены сжатые данные!\n");
	}
	...
	{
		if (verbose)
			printf("ВНИМАНИЕ: Не верный хэш секции метаданных!\n");
	}

	// Checking ECDSA signatures.
	if ((edat->flags & EDAT_DEBUG_DATA_FLAG) == 0)
	{
		printf("Проверка подписей...\n");
		...
		if ((!memcmp(signature_r, zero_buf, 0x15)) || (!memcmp(signature_s, zero_buf, 0x15)))
			printf("Не верная подпись метаданных!\n");
		else
		{
			...
			if (!ecdsa_verify(signature_hash, signature_r, signature_s))
			{
				...
				if (((unsigned long long)edat->block_size * block_num) > 0x100000000)
					printf("*Из-за большого размера файла, статус подписи метаданных может быть некорректным!\n");
			}
			else
				printf("Подпись метаданных верна!\n");
		}
		...
		if ((!memcmp(signature_r, zero_buf, 0x15)) || (!memcmp(signature_s, zero_buf, 0x15)))
			printf("Подпись заголовка не верна!\n");
		else
		{
			...
			if (ecdsa_verify(signature_hash, signature_r, signature_s))
				printf("Подпись заголовка верна!\n");
			else
				printf("Подпись заголовка не верна!\n");
		}
	}
...
int validate_npd_hashes(const char* file_name, unsigned char *klicensee, NPD_HEADER *npd, bool verbose)
{
	...
	if (verbose)
	{
		if (title_hash_result)
			printf("Хэш названия NPD верный!\n");
		else
			printf("ВНИМАНИЕ: Хэш названия NPD не верный!\n");
	}
	...
	else
	{
		...
		{
			if (dev_hash_result)
				printf("Dev хэш NPD верный!\n");
			else
				printf("ВНИМАНИЕ: Dev хэш NPD не верный!\n");
		}
	}
...
bool extract_data(FILE *input, FILE *output, const char* input_file_name, unsigned char* devklic, unsigned char* rifkey, bool verbose)
{
	...
	{
		printf("Ошибка: Файл имеет недопустимый заголовок NPD.");
		return 1;
	}
	...
	printf("NPD ЗАГОЛОВОК\n");
	printf("NPD версия: %d\n", NPD->version);
	printf("NPD лицензия: %d\n", NPD->license);
	printf("NPD тип: %x\n", NPD->type);
	printf("NPD контент ID: %s\n", NPD->content_id);
	printf("\n");
	...
	{
		printf("SDAT ЗАГОЛОВОК\n");
		printf("SDAT флаги: 0x%08X\n", EDAT->flags);
		printf("SDAT размер блока: 0x%08X\n", EDAT->block_size);
		printf("SDAT размер файла: 0x%llX\n", EDAT->file_size);
		printf("\n");
	...
	{
		printf("EDAT ЗАГОЛОВОК\n");
		printf("EDAT флаги: 0x%08X\n", EDAT->flags);
		printf("EDAT размер блока: 0x%08X\n", EDAT->block_size);
		printf("EDAT размер файла: 0x%llX\n", EDAT->file_size);
		printf("\n");
		...
		if (!validate_npd_hashes(real_file_name, devklic, NPD, verbose))
		{
			...
			if ((EDAT->flags & EDAT_DEBUG_DATA_FLAG) != EDAT_DEBUG_DATA_FLAG)
			{
				printf("Ошибка: Не удалось проверить хэш NPD!\n");
				return 1;
			}
		}
			...
			|| ((NPD->license & 0x1) == 0x1))      // Type 1: Use network activation (RIF key).
		{
			...
			if (!test)
			{
				printf("Ошибка: Необходим действительный файл RAP/RIF для этого файла EDAT!");
				return 1;
			}
		}
	...
	{
		int i;
		printf("КЛЮЧ ДЕКРИПТОВКИ: ");
		for (i = 0; i < 0x10; i++)
			printf("%02X", key[i]);
		printf("\n\n");
	}

	printf("Парсинг данных...\n");
	if (check_data(key, EDAT, NPD, input, verbose))
	{
		printf("Парсинг не удался!\n");
		return 1;
	}
	else
		printf("Файл успешно проанализирован!\n");

	printf("\n");

	printf("Декриптовка данных...\n");
	if (decrypt_data(input, output, EDAT, NPD, key, verbose))
	{
		printf("Декриптовка не удалась!");
		return 1;
	}
	else
...
int encrypt_data(FILE *in, FILE *out, EDAT_HEADER *edat, NPD_HEADER *npd, unsigned char* crypt_key, bool verbose)
{
	...
	for (i = 0; i < block_num; i++)
	{
		...
		else
		{
			...
			{
				if (verbose)
					printf("ВНИМАНИЕ: Блок по адресу 0x%08x имеет не верный фейковый хэш!\n", offset);
			}
		}
...
bool pack_data(FILE *input, FILE *output, const char* input_file_name, unsigned char* content_id, unsigned char* devklic, unsigned char* rifkey, int version, int license, int type, int block, bool useCompression, bool isEDAT, bool isFinalized, bool verbose)
{
	...
	if (version == 1)
	{
		...
		{
			printf("Ошибка: Не верная версия для SDAT!");
			return 1;
		}
	}
	else if (version == 2)
	...
	else
	{
		...
		else if ((NPD->license & 0x2) == 0x2)   // Type 2: Use key from RAP file (RIF key).
		{
			...
			if (!test)
			{
				printf("ОШИБКА: Необходим действительный файл RAP/RIF для этого EDAT файла!");
				return 1;
			}
		}
		else if ((NPD->license & 0x1) == 0x1)    // Type 1: Use network activation.
		{
			printf("ОШИБКА: Сетевая лицензия не поддерживается!");
			return 1;
		}
	...
	if (isFinalized)
	{
		printf("Подделка данных...\n");
		if (forge_data(key, EDAT, NPD, output))
		{
			printf("Подделка не удалась!\n");
			return 1;
		}
		else
			printf("Файл успешно подделан!\n");
	}
...
void print_usage()
{
	...
	printf("                   - Trnsltd by ErikP (C).\n\n");
	printf("***************************************************************************\n\n");
	printf("Применение: make_npdata [-v] -e <вход> <выход> <формат> <данные> <версия>\n");
	printf("                                <сжатие> <блок> <лицензия> <тип> <cID>\n");
	printf("                                <klic> <rap/rif>\n");
	printf("            make_npdata [-v] -d <вход> <выход> <klic> <rap/rif>\n");
	printf("            make_npdata [-v] -b <вход> <источник> <режим>\n\n");
	printf("- Режимы:\n");
	printf("[-v]: Verbose mode    : Режим подробного протоколирования\n");
	printf("[-e]: Encryption mode : Режим шифрования\n");
	printf("[-d]: Decryption mode : Режим дешифрования\n");
	printf("[-b]: Bruteforce mode : Режим 'Грубой силы'\n");
	printf("\n");
	printf("- Только режим шифрования:\n");
	printf("             <формат> : 0 - SDAT\n");
	printf("                        1 - EDAT\n");
	printf("             <данные> : 0 - Дебаг данные\n");
	printf("                        1 - Финализированные данные\n");
	printf("             <версия> : 1 - EDAT версия 1\n");
	printf("                        2 - EDAT/SDAT версия 2\n");
	printf("                        3 - EDAT/SDAT версия 3\n");
	printf("                        4 - EDAT/SDAT версия 4\n");
	printf("             <сжатие> : 0 - Отключить сжатие\n");
	printf("                        1 - Включить сжатие\n");
	printf("               <блок> : Размер блока в Кб (1, 2, 4, 8, 16, 32)\n");
	printf("\n");
	printf("- Только финализированный EDAT:\n");
	printf("           <лицензия> : 1 - Сетевая лицензия (не поддерживается)\n");
	printf("                        2 - Локальная лицензия (использовать RAP файл как ключ)\n");
	printf("                        3 - Свободная лицензия (использовать klic файл как ключ)\n");
	printf("                <тип> : 00 - Общий\n");
	printf("                        01 - PS2 EDAT\n");
	printf("                        20 - PSP Ремастер\n");
	printf("                        21 - Модули (привязка к диску)\n");
	printf("                        30 - Неизвестно\n");
	printf("                <cID> : Контент ИД (XXYYYY-AAAABBBBB_CC-DDDDDDDDDDDDDDDD)\n");
	printf("\n");
	printf("- Режимы шифрования и дешифрования\n");
	printf("               <klic> : 0 - No key\n");
	printf("                        1 - NPDRM OMAC key 1 (Ключ свободной лицензии)\n");
	printf("                        2 - NPDRM OMAC key 2\n");
	printf("                        3 - NPDRM OMAC key 3\n");
	printf("                        4 - PS3 key (klic_dec_key)\n");
	printf("                        5 - PSX key (PSOne Классика)\n");
	printf("                        6 - PSP key 1 (PSP Минис)\n");
	printf("                        7 - PSP key 2 (PSP Ремастер)\n");
	printf("                        8 - Кастомный ключ (считать из входящего или klic.bin файла)\n");
	printf("            <rap/rif> : RAP файл для де/шифровки или rifkey.bin (опционально)\n");
	printf("\n");
	printf("- Режим 'Грубой силы' - Магия последовательного перебора\n");
	printf("           <источник> : Исходный ELF файл для klic\n");
	printf("              <режим> : 0 - Бинарный\n");
	printf("                        1 - Текстовой\n");
	printf("                        2 - Юникодовый текст\n");
}
...
int main(int argc, char **argv)
{
	...
	if ((!strcmp(argv[arg_offset + 1], "-e")) && (argc > (arg_offset + 7)))
	{
		...
		{
			printf("ОШИБКА: Не верные параметры!\n");
			fclose(input);
			return 0;
		}
		...
		if (format && data)
		{
			...
			{
				printf("ОШИБКА: Не хватает параметров для финализации EDAT!\n");
				fclose(input);
				return 0;
			}
			...
			{
				printf("ОШИБКА: Не верные параметры финализации EDAT!\n");
				fclose(input);
				return 0;
			}
			...
			case 8:
			{
				...
				{
					printf("ОШИБКА: Пожалуйста, разместите бинарный кастомный klic в файле klic.bin!\n");
					fclose(input);
					fclose(klic_file);
					return 0;
				}
			...
			default:
				printf("ОШИБКА: Не верный режим klic!\n");
				fclose(input);
				return 0;
			}
			...
			if (argv[arg_offset + 12] != NULL)
			{
				...
				if (!strcmp(real_rap_name, "rifkey.bin"))
				{
					if (rap == NULL)
					{
						printf("ОШИБКА: Пожалуйста, разместите ваш ключ RIF в файле rifkey.bin!\n");
						fclose(input);
						return 0;
					}
				...
				else
				{
					if (rap == NULL)
					{
						printf("ОШИБКА: Пожалуйста, разместите ваш ключ RAP в файл rap!\n");
						fclose(input);
						return 0;
					}
	...
	else if (!strcmp(argv[arg_offset + 1], "-d") && (argc > (arg_offset + 4)))
	{
		...
		if (input == NULL)
		{
			printf("ОШИБКА: Пожалуйста, проверьте ваш входящий файл!\n");
			return 0;
		}
		...
		case 8:
		{
			...
			{
				printf("ОШИБКА: Пожалуйста, положите кастомный klic в файл klic.bin!\n");
				fclose(input);
				fclose(klic_file);
				return 0;
			}
		...
		if (argv[arg_offset + 4] != NULL)
		{
			...
			if (!strcmp(real_rap_name, "rifkey.bin"))
			{
				if (rap == NULL)
				{
					printf("ОШИБКА: Пожалуйста, положите ваш бинарный ключ RIF в файл rifkey.bin!\n");
					fclose(input);
					return 0;
				}
				fread(rifkey, sizeof(rifkey), 1, rap);
			...
			else
			{
				if (rap == NULL)
				{
					printf("ОШИБКА: Пожалуйста, положите ваш ключ RAP в файл rap!\n");
					fclose(input);
					return 0;
				}
				fread(rapkey, sizeof(rapkey), 1, rap);
	...
	else if (!strcmp(argv[arg_offset + 1], "-b") && (argc > (arg_offset + 3)))
	{
		...
		if (input == NULL)
		{
			printf("ОШИБКА: Пожалуйста, проверьте ваш входной файл!\n");
			return 0;
		}

		if (source == NULL)
		{
			printf("ОШИБКА: Пожалуйста, проверьте ваш исходный файл!\n");
			fclose(input);
			return 0;
		}
		...
		if (!memcmp(magic, sce_magic, 4))
		{
			...
			if (verbose)
			{
				printf("SCE файл обнаружен!\n");
				printf("NPD смещение в SCE: 0x%08x\n", npd_offset);
			}
		}
		...
		if (verbose)
		{
			if (mode == 1)
				...
				printf("РЕЖИМ: Текст в юникоде\n");
			else
				printf("РЕЖИМ: Бинарный\n");
		...
		if (found)
		{
			...
			printf("Не удалось сбрутфорсить klic!\n");
